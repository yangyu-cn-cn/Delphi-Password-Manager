...
type
  TFrmEntryList = class(TForm)
    ...
    LblEmail: TLabel;
    EdtEmail: TEdit;
    CebEmail: TClearEditButton;
    LblMPhon: TLabel;
    EdtMPhon: TEdit;
    CebMPhon: TClearEditButton;
    LblIMess: TLabel;
    EdtIMess: TEdit;
    CebIMess: TClearEditButton;
    LinkControlToField7: TLinkControlToField;
    LinkControlToField8: TLinkControlToField;
    LinkControlToField10: TLinkControlToField;
    SbtCopEmail: TSpeedButton;
    SbtCopMPhon: TSpeedButton;
    SbtCopIMess: TSpeedButton;
    ...
    procedure SbtCopEmailClick(Sender: TObject);
    procedure SbtCopMPhonClick(Sender: TObject);
    procedure SbtCopIMessClick(Sender: TObject);
  ...
  private
    ...
    procedure ModifyButtonStatus(bStatus: boolean);
    procedure EditReadOnlyStatus(bStatus: boolean);
...
procedure TFrmEntryList.ModifyButtonStatus(bStatus: boolean);
begin
  ImcIcon.Enabled   := bStatus;
  SpbCancel.Visible := bStatus;
  SpbSave.Visible   := bStatus;
  CebUser.Visible   := bStatus;
  CebURL.Visible    := bStatus;
  CebEmail.Visible  := bStatus;
  CebMPhon.Visible  := bStatus;
  CebIMess.Visible  := bStatus;
  EdbPasGen.Visible := bStatus;
end;

procedure TFrmEntryList.EditReadOnlyStatus(bStatus: boolean);
begin
  EdtTitel.ReadOnly := bStatus;
  EdtUserN.ReadOnly := bStatus;
  EdtPassw.ReadOnly := bStatus;
  EdtURL.ReadOnly   := bStatus;
  EdtEmail.ReadOnly := bStatus;
  EdtMPhon.ReadOnly := bStatus;
  EdtIMess.ReadOnly := bStatus;
  EdtNotes.ReadOnly := bStatus;
  DteDateC.ReadOnly := bStatus;
end;
...
begin
  ...
  ModifyButtonStatus((nStatus = 2) or (nStatus = 3));
  {$IF DEFINED(iOS) or DEFINED(ANDROID)}
  EditReadOnlyStatus((nStatus = 0) or (nStatus = 1));
  {$ENDIF}
end;

procedure ClipboardX(str: string);
var
  Svc: IFMXClipboardService;
begin
  if TPlatformServices.Current.SupportsPlatformService(IFMXClipboardService, Svc) then
    Svc.SetClipboard(str);
end;

procedure TFrmEntryList.SbtCopEmailClick(Sender: TObject);
begin
  ClipboardX(EdtEmail.Text);
end;

procedure TFrmEntryList.SbtCopIMessClick(Sender: TObject);
begin
  ClipboardX(EdtIMess.Text);
end;

procedure TFrmEntryList.SbtCopMPhonClick(Sender: TObject);
begin
  ClipboardX(EdtMPhon.Text);
end;
